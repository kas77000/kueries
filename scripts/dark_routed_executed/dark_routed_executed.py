...
def test_a_null_number_arrives_as_nan():
    ...
    pandas internals - which is what one null number on one Japanese name did to
    a whole quarter of the sister report, on the date it first turned up.
    ...
    class Result:
        def pd(self):
            ...
            df["shares_routed"] = pd.Series(masked)
            return df

    df = _to_pandas(Result())

    assert not isinstance(df["shares_routed"].values, np.ma.MaskedArray), "shares_routed is still masked"
    assert df["shares_routed"].dtype == np.float64, df["shares_routed"].dtype
    assert isinstance(df["fxlast"].dtype, np.dtype), "fxlast is still an extension type"
    assert df["shares_routed"].notna().tolist() == [True, False, True]
    assert df["fxlast"].notna().tolist() == [True, False, True]
    assert df["shares_routed"].iloc[0] == 1_000_000.0 and np.isnan(df["shares_routed"].iloc[1])
