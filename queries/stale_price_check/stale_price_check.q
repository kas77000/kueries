...
/   q)stalePriceCheck[h;0;0]                   / calibration: everything, ranked
/   q)stalePriceCheck[h;25;5000]               / alert: >25bps, or print >5s old
/
...
/
/ A threshold of 0 turns that test OFF.  stalePriceCheck[h;0;0] is therefore
/ the calibration run: every workorder under a live parent, every number filled
/ in, flag reading `ok on all of them because no threshold was set.  Read
/ dev_bps and price_age_ms directly on that run, decide what "bad" looks like
/ on this book, then pass those numbers.
...
stalePriceCheck:{[h;minDevBps;minAgeMs]
  ...
  f:{[d]
    ...
    w:select date,id_server,id_target,id_work,sequence,trader,sym,side,size,
      ...
      from workorder0 where date=d, id_target in ids;
